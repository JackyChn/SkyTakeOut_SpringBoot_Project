...
//        check the order in db first
        Orders order = orderMapper.getById(id);
        if (order == null) {
            throw new OrderBusinessException(MessageConstant.ORDER_NOT_FOUND);
        }

//        send message to admin web browser msg that user has reminded the order
        Map mp = new HashMap();
        mp.put("type", 2);  // 2 represents for user reminder
        mp.put("orderId", id);
        mp.put("content", "orderNo: " + order.getNumber());
